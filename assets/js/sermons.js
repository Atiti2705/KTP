  ...
  listContainer.innerHTML = paginationData.items.map(sermon => `
    <div class="doc-card reveal" style="position: relative;">
      ...
      <label class="item-select-label" style="position: absolute; top: var(--sp-2); left: var(--sp-2); z-index: 5; background: rgba(0,0,0,0.05); border-radius: var(--radius-sm); padding: 4px; display: flex; cursor: pointer;" onclick="event.stopPropagation();">
        <input type="checkbox" class="item-checkbox" data-url="${sermon.fileUrl && sermon.fileUrl !== '#' ? sermon.fileUrl : ''}" data-name="${sermon.title}.${sermon.fileType.toLowerCase()}" style="width: 18px; height: 18px; cursor: pointer;">
      </label>
...
function setupBulkDownload() {
  ...
  if (listContainer) {
    listContainer.addEventListener('change', (e) => {
      if (e.target.classList.contains('item-checkbox')) {
        ...
        if (e.target.checked) {
          selectedSermons.add(JSON.stringify({url, name}));
        } else {
          selectedSermons.delete(JSON.stringify({url, name}));
        }
        updateBulkUI();
      }
    });
    ...
    try {
      ...
      const promises = files.map(async (file, index) => {
        try {
          ...
          const baseName = file.name.substring(0, file.name.lastIndexOf('.')) || `sermon_${index}`;
          const finalName = `${baseName}_${index}.${ext}`;
          
          zip.file(finalName, blob);
        } catch (err) {
          console.error("Failed to fetch sermon for zip:", file.url, err);
        }
      });
  ...
  function updateBulkUI() {
    countSpan.textContent = selectedSermons.size;
    if (selectedSermons.size > 0) {
      btnDownload.disabled = false;
