  ...
  const itemsHtml = paginationData.items.map(photo => {
    ...
    return `
      <div class="masonry-item gallery-item selectable-item" data-id="${photo.id}" data-url="${photo.imageUrl}" data-name="${photo.title || 'photo'}.jpg" style="position: relative;">
        <img src="${photo.imageUrl}" alt="${photo.title}" class="gallery-image" loading="lazy" onerror="this.onerror=null; this.src='assets/images/logo.png';">
      </div>
    `;
  }).join('');
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          if (window.Capacitor && window.Capacitor.isNativePlatform()) {
            if (window.NativeDownload) {
              await window.NativeDownload(file.url, file.name || `photo_${i}.jpg`);
            } else {
